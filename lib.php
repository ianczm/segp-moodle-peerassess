...
define('FEEDBACK_RESETFORM_RESET', 'peerassess_reset_data_');
define('FEEDBACK_RESETFORM_DROP', 'peerassess_drop_peerassess_');
...
function peerassess_pluginfile($course, $cm, $context, $filearea, $args, $forcedownload, array $options=array()) {
    ...
    if ($filearea === 'page_after_submit' or $filearea === 'item') {
        ...
        if (isset($CFG->peerassess_allowfullanonymous)
                    ...
                    AND $peerassess->anonymous == FEEDBACK_ANONYMOUS_YES ) {
            $canload = true;
        }
...
function peerassess_user_outline($course, $user, $mod, $peerassess) {
    ...
    if ($peerassess->anonymous != FEEDBACK_ANONYMOUS_NO) {
...
function peerassess_reset_course_form_definition(&$mform) {
    ...
    foreach ($peerassesss as $peerassess) {
        $mform->addElement('checkbox', FEEDBACK_RESETFORM_RESET.$peerassess->id, $peerassess->name);
    }
}
...
function peerassess_reset_course_form_defaults($course) {
    ...
    foreach ($peerassesss as $peerassess) {
        $return[FEEDBACK_RESETFORM_RESET.$peerassess->id] = true;
    }
    return $return;
}
...
function peerassess_reset_course_form($course) {
    ...
    foreach ($peerassesss as $peerassess) {
        ...
        echo html_writer::checkbox(FEEDBACK_RESETFORM_RESET.$peerassess->id,
                                1, true,
                                get_string('resetting_data', 'peerassess'));
        echo '<br />';
        echo html_writer::checkbox(FEEDBACK_RESETFORM_DROP.$peerassess->id,
                                1, false,
...
function peerassess_get_group_values($item,
    ...
    if ($DB->get_field('peerassess', 'anonymous', $params) == FEEDBACK_ANONYMOUS_YES) {
        if (is_array($values)) {
            shuffle($values);
        }
    }
...
function peerassess_extend_settings_navigation(settings_navigation $settings,
    ...
    if (has_capability('mod/peerassess:viewreports', $context)) {
        ...
        if ($peerassess->anonymous == FEEDBACK_ANONYMOUS_NO AND $peerassess->course != SITEID) {
